            ...
            @foreach($profile as $key2 => $data2)
                @foreach($resume as $key3 => $data3)
                    ...
                    @if($data3->OBJECTIVE != NULL && $data3->ADDRESS != NULL && $data3->EXPERIENCE != NULL &&
                        ...
                        <div align="center">
                            <form action="/Phlick/PDF/open/{{Session::get('ID')}}/0" method="POST" target="_blank">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                <button class="btn btn-success" type="submit">View PDF</button>
                                <a class="btn btn-primary" href="/Phlick/PDF/download/{{Session::get('ID')}}/0">Download PDF</a>
                            </form>
                            <br><br>
                        </div>
                    @endif
